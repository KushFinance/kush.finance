...
contract kSEEDVoting {
  ...
  modifier _updatePeriods() {
    if (
      block.number >= currentVotingStartBlock &&
      block.number <= currentVotingEndBlock
    ) {
      isVotingPeriod = true;
    } else {
    ...
    if (
      ...
      block.number > currentDistributionEndBlock
    ) {
  ...
  event ClaimDistribution(
    address indexed claimer,
    uint256 amountClaimed,
    uint256 kseedStaked,
    uint256 kkushLPStaked
  );
  ...
  function setConnector(address _connector)
    public
    _onlyConnector
    _updatePeriods
  {
  ...
  function setIsRewardingkKush(bool _isRewarding)
    public
    _onlyConnector
    _updatePeriods
  {
  ...
  function setVotingPeriodBlockLength(uint256 _blocks)
    public
    _onlyConnector
    _updatePeriods
  {
  ...
  function setSafetyWithdrawal(bool _safetyBool)
    public
    _onlyConnector
    _updatePeriods
  {
  ...
  function setkSeedFarmAddress(address _addr)
    public
    _onlyConnector
    _updatePeriods
  {
  ...
  function addChainBid(
    ...
    require(
      keccak256(bytes(userBid)) == keccak256(bytes(bidId)),
      'This is not your bid'
    );
    //verify this
    if (keccak256(bytes(bidChains[id].id)) == keccak256(bytes(id))) {
      require(
        keccak256(bytes(currentBids[msg.sender].bidId)) ==
          keccak256(bytes(bidChains[id].bidId))
      );
    } else {
      require(
        keccak256(bytes(bidChains[id].id)) != keccak256(bytes(id)),
    ...
    for (uint256 i = 0; i < currentBids[msg.sender].chain.length; i++) {
      if (
        keccak256(bytes(currentBids[msg.sender].chain[i])) ==
        keccak256(bytes(id))
      ) {
        bidExists = true;
      }
  ...
  function voteForBid(address _bidAddr, uint256 _votes) public _updatePeriods {
    ...
    votedkSeed[msg.sender].kseedLocked = votedkSeed[msg.sender].kseedLocked.add(
      _votes
    );
    ...
    if (
      (currentBids[_bidAddr].votes > topBidVotes) && (topBidAddress != _bidAddr)
    ) {
  ...
  function getBid(address _address)
    public
    view
    returns (
      string memory,
  ...
  function getChain(string memory id)
    public
    view
    returns (
      string memory,
  ...
  function determinekKushCost(address voter, uint256 votes)
    public
    view
    returns (uint256)
  {
  ...
  function withdrawBidkSeed() public _updatePeriods {
    require(
      (votedkSeed[msg.sender].votingRound < currentVotingRound) ||
        (votedkSeed[msg.sender].releaseBlock < block.number) ||
        (safetyWithdrawal),
      'kSEED is still locked for vote'
    );
  ...
  function executeBid() public _updatePeriods {
    require(
      currentVotingEndBlock < block.number,
    ...
    emit BidExecution(
      currentBids[topBidAddress].bidder,
      currentBids[topBidAddress].bidId
    );
  }
  ...
  function distributeFunds(address _addr, uint256 _amount)
    public
    _onlyConnector
    _updatePeriods
  {
    ...
    require(
      (currentDistributionEndBlock < block.number),
      'Distribution funds error'
    );
    //Check that it has been more than 1 day since last distribution
    require(
      (block.number - currentDistributionEndBlock) > distributionPeriodBuffer,
  ...
  function claimDistribution(
    address _claimer,
    uint256 _kseedAmount,
    uint256 _kKushLPAmount
  ) public {
    require(
      isDistributing && currentVotingEndBlock > block.number,
      'You are not in a distribution period'
    );
    kseedIERC20.safeTransferFrom(_claimer, address(this), _kseedAmount);
    claims[_claimer].kseedLocked = claims[_claimer].kseedLocked.add(
      _kseedAmount
    );
    ...
    currentDistributionAmountClaimed = currentDistributionAmountClaimed.add(
      claimedAmount
    );
    emit ClaimDistribution(
      _claimer,
      claimedAmount,
      _kseedAmount,
      _kKushLPAmount
  ...
  function withdrawDistributionkSeed() public {
    require(
      (!isDistributing) || (safetyWithdrawal),
      'Locked kSEED  can only be claimed outside of distribution period'
    );
    kseedIERC20.safeTransfer(msg.sender, claims[msg.sender].kseedLocked);
    claims[msg.sender].kseedLocked = claims[msg.sender].kseedLocked.sub(
      claims[msg.sender].kseedLocked
    );
  ...
  function burnkKush() public _updatePeriods {
    ...
    ERC20(kkushAddress).transfer(
      0x0000000000000000000000000000000000000001,
      halfkKush - 10
    );
    //send swapped kOG to kSEED LP farming contract
    ERC20(kOGAddress).transfer(
      kseedLPFarm,
      ERC20(kOGAddress).balanceOf(address(this))
    );
