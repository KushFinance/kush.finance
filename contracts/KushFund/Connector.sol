...
contract Connector {
  ...
  function interfaceYearn(
    ...
    if (
      ...
      keccak256(bytes('yosaGetExpectedReturn'))
    ) {
      YOneSplitAudit oneSplitAudit = YOneSplitAudit(_addresses[0]);
      // Needs to accept uint arrays
      oneSplitAudit.getExpectedReturn(
        _addresses[1],
        _addresses[2],
        integers[0],
        integers[1],
    ...
    if (
      keccak256(bytes(functionName)) == keccak256(bytes('yStratWithdrawAll'))
    ) {
      YearnStrategy yStrategy = YearnStrategy(_addresses[0]);
    ...
    if (
      keccak256(bytes(functionName)) ==
      keccak256(bytes('yERCPricePerFullShare'))
    ) {
      YearnERC20 yERC = YearnERC20(_addresses[0]);
    ...
    if (
      keccak256(bytes(functionName)) == keccak256(bytes('yVaultsDepositAll'))
    ) {
      YearnVault yVaults = YearnVault(_addresses[0]);
      yVaults.depositAll();
    }
    ...
    if (
      keccak256(bytes(functionName)) == keccak256(bytes('yVaultsWithdrawAll'))
    ) {
      YearnVault yVaults = YearnVault(_addresses[0]);
      yVaults.withdrawAll();
    }
    if (
      keccak256(bytes(functionName)) ==
      keccak256(bytes('yVaultsPricePerFullShare'))
    ) {
      YearnVault yVaults = YearnVault(_addresses[0]);
