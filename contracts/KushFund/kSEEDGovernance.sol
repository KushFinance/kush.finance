...
contract KushGovernance {
    ...
    uint256 public costPerVote = 42**18;
    ...
    modifier _isAdmin() 
    {
        require(currentAdmins[msg.sender].isAdmin, "You do not have permission.");       _;
    }
    ...
    function createProposal(string memory _proposalQuestion,address _proposer,string[] memory _options,  string[] memory _optionImgs,    uint256 _endBlock) public 
    ...
    function getProposal(address _proposal) public view returns(string memory,address, string[] memory, string[] memory, uint256[] memory,uint256) 
                                            {
                                                return(eachProposal[_proposal].proposalQuestion, 
    ...
    function voteForProposal(uint256 _amount, uint256 _optionIndex, address _proposalAddr,uint256 _votes) public {
        require(block.number < eachProposal[_proposalAddr].endBlock, "Voting is already over!");    
        kseedIERC20.safeTransferFrom(msg.sender, address(this), _amount > costPerVote);
        kseedVotes[msg.sender].votes = kseedVotes[msg.sender].votes.add(_amount);
        kseedVotes[msg.sender].proposalAddr = _proposalAddr;
        kseedVotes[msg.sender].proposalQuestion = eachProposal[_proposalAddr].proposalQuestion;
        kseedVotes[msg.sender].option = eachProposal[_proposalAddr].options[_optionIndex];
        eachProposal[_proposalAddr].optionVotes[_optionIndex] = eachProposal[_proposalAddr].optionVotes[_optionIndex].add(_amount);
    ...
    function removeAdmin(address _admin) public _isAdmin {currentAdmins[_admin].isAdmin = false;
